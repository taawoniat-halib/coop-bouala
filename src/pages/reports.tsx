...
import { Printer, Calendar, CheckCircle2, XCircle, Loader2, Send, Search,
  Download, FileSpreadsheet, FileText } from 'lucide-react';
...
export default function Reports() {
  ...
  const memberStatements = useMemo(() =>
    computeMemberMonthlyStatements(members, receipts, transporters, prices, monthFilter, milkPurchasePrice, milkSellPrice)
  ...
  const farmerReport = useMemo(() => {
    ...
    const grossAmount = farmerReceipts.reduce((s, r) => s + r.quantityLiters * (r.pricePerLiter ?? monthPrice), 0);
    ...
    return { member, totalLiters, transportCost, grossAmount, netAmount, purchaseValue, sellValue, profit, farmerReceipts };
  }, [selectedFarmerIdReport, members, advancedReceipts, prices, monthFilter, milkPurchasePrice, milkSellPrice]);
  ...
  const handleShareFarmerReport = () => {
    ...
    const phone = member.phone || settings?.phone;
    const period = advancedFromDate && advancedToDate
  ...
  const handleExportGridPdf = () => {
    ...
    const rows = gridMembers.map(m => {
      ...
      days.forEach(d => { row[`d${d}`] = dayMap.get(d) ? dayMap.get(d)!.toFixed(1) : ''; });
      return row;
    });
    exportToPdf(`التقرير الإجمالي — ${monthLabel(monthFilter)}`, columns, rows, `تقرير-إجمالي-${monthFilter}`);
  };
  ...
  const handleExportGridExcel = () => {
    ...
    const rows = gridMembers.map(m => {
      ...
      days.forEach(d => { row[`d${d}`] = dayMap.get(d) ?? ''; });
      return row;
    });
    exportToExcel(`إجمالي ${monthLabel(monthFilter)}`, columns, rows, `تقرير-إجمالي-${monthFilter}`);
  };
  ...
  const handleExportFarmerPdf = () => {
    ...
    exportToPdf(
      ...
      [
        ...
        { label: 'الفترة', value: advancedFromDate && advancedToDate ? `${advancedFromDate} → ${advancedToDate}` : monthLabel(monthFilter) },
  ...
  const handleExportFarmerExcel = () => {
    ...
    exportToExcel(
      ...
      [{ header: 'التاريخ', key: 'date' }, { header: 'الكمية (ل)', key: 'qty' }, { header: 'الثمن/ل', key: 'price' }, { header: 'النقل', key: 'transport' }],
    ...
    <Layout>
              ...
              {allMembersSorted.map(m => {
                ...
                return (
                  ...
                  >
                    ...
                    {hasData
                      ? <Badge variant="outline" className="text-[10px] shrink-0 bg-emerald-50 text-emerald-700 border-emerald-200">لديه بيانات</Badge>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">مستحقات الفلاحين وتسليمات الشركات والتقارير التفصيلية</p>
        </div>
            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportMembersPdf}>
                    <FileText className="h-3.5 w-3.5" /> PDF
                  </Button>
                  <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportMembersExcel}>
                        ...
                        <TableRow key={st.memberId}
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {isToggling ? <Loader2 className="h-3 w-3 animate-spin" /> : paid ? 'إلغاء الدفع' : 'تحديد كمدفوع'}
                              </Button>
              ...
              <CardContent>
                <div className="flex flex-wrap items-end gap-4">
                  <div className="space-y-1">
                    ...
                    <Input type="date" value={advancedFromDate}
                      onChange={(e) => setAdvancedFromDate(e.target.value)} dir="ltr" className="w-44" />
                  </div>
                  <div className="space-y-1">
                    <Label>إلى تاريخ</Label>
                    <Input type="date" value={advancedToDate}
                      onChange={(e) => setAdvancedToDate(e.target.value)} dir="ltr" className="w-44" />
                  </div>
                  {(advancedFromDate || advancedToDate) && (
                    <Button variant="outline" size="sm" onClick={() => { setAdvancedFromDate(''); setAdvancedToDate(''); }}>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between flex-wrap gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportGridPdf}>
                      <FileText className="h-3.5 w-3.5" /> PDF
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportGridExcel}>
                      <FileSpreadsheet className="h-3.5 w-3.5" /> Excel
                    </Button>
                  </div>
                </div>
                    ...
                    <table className="w-full text-sm border-collapse">
                      <thead>
                        <tr className="bg-muted/50">
                          <th className="border border-border px-2 py-2 text-right font-semibold min-w-[120px]">#</th>
                          <th className="border border-border px-2 py-2 text-right font-semibold min-w-[160px]">الفلاح</th>
                          {days.map(d => (
                            <th key={d} className="border border-border px-1 py-2 text-center font-semibold min-w-[32px] text-xs">{d}</th>
                          ))}
                          <th className="border border-border px-2 py-2 text-center font-semibold bg-primary/10 min-w-[70px]">المجموع</th>
                        </tr>
                      </thead>
                      <tbody>
                        {gridMembers.map((m, idx) => {
                          ...
                          return (
                            <tr key={m.id} className={idx % 2 === 0 ? 'bg-background' : 'bg-muted/20'}>
                              <td className="border border-border px-2 py-1.5 text-center text-xs text-muted-foreground">{idx + 1}</td>
                              <td className="border border-border px-2 py-1.5 font-medium">{m.fullName}</td>
                              {days.map(d => {
                                const val = dayMap.get(d);
                                return (
                                  <td key={d} className="border border-border px-1 py-1.5 text-center font-mono text-xs">
                                    {val ? val.toFixed(1) : ''}
                                  </td>
                                );
                              })}
                        ...
                        <tr className="bg-muted/50 font-bold">
                          <td className="border border-border px-2 py-2 text-center" colSpan={2}>المجموع</td>
                          {days.map(d => {
                            const dayTotal = gridMembers.reduce((s, m) => s + (memberDailyGrid.get(m.id)?.get(d) ?? 0), 0);
                            return (
                              <td key={d} className="border border-border px-1 py-2 text-center font-mono text-xs">
                                {dayTotal > 0 ? dayTotal.toFixed(1) : ''}
                              </td>
                            );
                          })}
                          <td className="border border-border px-2 py-2 text-center font-mono text-primary bg-primary/10">
                            {gridMembers.reduce((s, m) => {
                              const dayMap = memberDailyGrid.get(m.id) ?? new Map();
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between flex-wrap gap-3">
                  ...
                  {farmerReport && (
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportFarmerPdf}>
                        <FileText className="h-3.5 w-3.5" /> PDF
                      </Button>
                      <Button variant="outline" size="sm" className="gap-1.5" onClick={handleExportFarmerExcel}>
                        <FileSpreadsheet className="h-3.5 w-3.5" /> Excel
                      </Button>
              ...
              <CardContent className="space-y-4">
                ...
                {farmerReport && (
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { label: 'اسم الفلاح', value: farmerReport.member.fullName, highlight: false },
                        { label: 'اسم المركز', value: settings?.coopName || '—', highlight: false },
                        { label: 'مجموع اللترات', value: `${farmerReport.totalLiters.toFixed(1)} ل`, highlight: true },
                        { label: 'مبلغ النقل المخصوم', value: `${farmerReport.transportCost.toFixed(2)} ${currency}`, highlight: false },
                        { label: 'الصافي الإجمالي', value: `${farmerReport.netAmount.toFixed(2)} ${currency}`, highlight: true },
                        { label: 'قيمة شراء الحليب', value: `${farmerReport.purchaseValue.toFixed(2)} ${currency}`, highlight: false },
                        { label: 'قيمة بيع الحليب', value: `${farmerReport.sellValue.toFixed(2)} ${currency}`, highlight: false },
                        { label: 'الربح المتوقع', value: `${farmerReport.profit.toFixed(2)} ${currency}`, highlight: false },
                      ].map(item => (
                        <div key={item.label}
                          className={`rounded-lg border p-3 ${item.highlight ? 'border-primary/30 bg-primary/5' : 'border-border bg-muted/20'}`}>
                          <p className="text-xs text-muted-foreground mb-1">{item.label}</p>
                          <p className={`font-bold text-sm font-mono ${item.highlight ? 'text-primary' : ''}`}>{item.value}</p>
                        </div>
                      ))}
                    </div>
                        ...
                        <TableBody>
                          {farmerReport.farmerReceipts
                            ...
                            .map(r => {
                              ...
                              return (
                                <TableRow key={r.id}>
                                  <TableCell className="font-mono text-sm" dir="ltr">{r.date}</TableCell>
                                  <TableCell className="font-mono">{r.quantityLiters.toFixed(1)}</TableCell>
                ...
                {selectedFarmerIdReport && !farmerReport && (
                  <p className="text-center py-8 text-muted-foreground">لا توجد بيانات لهذا الفلاح في الفترة المحددة</p>
                )}
              </CardContent>
            </Card>
          </div>
